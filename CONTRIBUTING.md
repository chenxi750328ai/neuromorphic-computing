...
### 计划先行（分支 / PR · 2026-07-24 起 · 总裁要求）

1. **先写计划，再开分支/PR**：特性分支名、拟开 PR 标题与目的、是否要人签、门禁期望，须先登记在 `docs/plans/`（或当日 `docs/每日计划.md`），**禁止**临时起 PR 再塞进「待批」。
2. **计划自评**：开写代码前，计划须过 gstack `/plan-eng-review`（或等价自评段落落盘）；未自评不开工。
3. **PR Summary 诚实**：合入的是「工具/证据」还是「宣称门禁 PASS」必须写清；`overall_ok=false` 可以合（交账），但**不得**表述成关口已过。
4. **一 PR 一事**：禁止把无关核盘文档、多条特性塞进同一 PR（PR#12 教训）。
5. **「待总裁批」≠「待合 PR」**：§8/§10 类人签与工程 PR 分列；不得捆成一条「请批」。
