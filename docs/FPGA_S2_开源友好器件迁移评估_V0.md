...
**状态**：评估落盘 + **本机 openXC7 实测**（blinky 出 bit+上板 PASS；LIF/CARRY **堵**；**未**改主路径）  
**关联**：`docs/plans/2026-07-28-fpga-sovereign-toolchain.md` · `docs/FPGA_工具链安全可控策略_V0.md` · [`FPGA_按项目目标_独立评审_Opus_V0.md`](./FPGA_按项目目标_独立评审_Opus_V0.md)  
**证据**：`fpga_z2_openxc7_try.json`（soft_ps7 PASS）· `fpga_z2_openxc7_try_carry.json`（FAIL）· `fpga_z2_openxc7_board_load.json` · `fpga_z2_openxc7_lif_probe.json`
...
| Yosys + 钉版本 Vivado 辅证 bit | **LIF 开源 P&R/出 bit** 未通 |
| blinky 开源上板 PASS | 产品级闭环（可复现锁、门仿/后仿、整网主权）未建 |
...
| Lattice **ECP5** | ✅ Yosys+nextpnr-ecp5 成熟 | 中大 | 须重做板级 | **无硬核 ARM**：现「fc 在 PS」架构不能原样搬；须先重做系统切分账，否则不得写「首选」 |
...
3. **触发条件**（须总裁书面，机读关键字）：硬主权要求无 Vivado **且** openXC7 对 `lif_step` 出 bit 在 **15 个工作日**内未 `stages.bitstream.ok=true`（见目标评审 T3-4）。触发后评估 **ECP5**（须含无硬核 ARM 的架构反作用账，不得空喊首选）。  
