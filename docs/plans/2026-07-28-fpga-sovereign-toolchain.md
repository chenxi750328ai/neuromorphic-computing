...
- [ ] Verilator（或等价开源）对 `lif_step.v` / `lif_step_axi_lite.v` 仿真 `--gate`
- [ ] RTL 人工检视清单落盘 + 意见文件（CR）
- [ ] `neuro-ci` 或 pre_merge：**开源仿真门 BLOCK**；Vivado **不**作为 CI 唯一依赖
- [ ] QA 记录增加 RTL-SIM / RTL-CR 行；未绿不得宣称「硬件逻辑质量已保证」
- [ ] 文档钉死：Vivado = 快验/对照后端
...
### Task 0 · 批注与 IPD 挂钩
- [x] 总裁口径落盘（本计划 + 裁决页）
- [ ] `QA_验收记录` / IPD 裁剪补充「RTL 开源门禁不可裁」脚注
- [ ] 与 F4 双通关系：双通功能 PASS **不豁免** S0

### Task 1 · Verilator 主门禁
- [ ] testbench（或 cocotb）驱动 `lif_step` / AXI-Lite 最小事务
- [ ] `scripts/phase4_fpga_lif_verilator_gate.py`（或 `.sh`）`--gate`；对照 `lif_step_fp`
- [ ] 证据 JSON 落 `docs/phase4_poc_evidence/`
- [ ] 挂 `qa-neuro-baseline` 或 `neuro-ci` 一步

### Task 2 · RTL 检视
- [ ] 清单：复位、Q16.16、溢出、握手 done、AXI-Lite 单拍假设、与 Python 语义差
- [ ] 检视意见 md；缺陷建档闭环
- [ ] QA 签字栏增加 RTL-CR

### Task 3 · Vivado 降级定位文档
- [ ] `docs/FPGA_工具链安全可控策略_V0.md`：信任根、版本钉扎、联网策略、禁止事项
- [ ] 更新总裁一页 / path_b：开发默认开源仿真，Vivado=快验

### Task 4 · PR
- [ ] 独立 PR 或续 #17 后继；Summary 写清「可控优先 / Vivado 辅」；不代合
