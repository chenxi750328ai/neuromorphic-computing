...
### P5-1 落地（2026-08-06 · 板待修并行）

| 项 | 状态 |
|----|------|
| 默认编码 | ✅ MNIST→泊松率编码（N-MNIST 目录可选探测，未下载不挡） |
| 脚本 | ✅ `scripts/phase5_event_loader.py` |
| smoke | ✅ `python3 scripts/phase5_event_loader.py --smoke --gate` |
...
| 项 | 状态 |
|----|------|
| `scripts/gwt0_protocol_gate.py --gate` | ✅ 对齐 K/D/k_active + 竞争写 |
| 人批「开 GWT-1」 | ☐ 待 |

## Phase4.2 下一刀（与 Phase5 并行 · **板待修暂停上板**）

- **P4.2-LAT**：单张 e2e ≤ **100 ms/张**（总裁已接受；当前 ≈306 s/张）  
- **阻塞**：PYNQ `192.168.137.3` 待修复；禁 `--skip-board` 假绿  
- 其它业务指标：**单独定标**，不混进本门  
- 工程：见 STALL 登记 + F7 延迟一页；离线 overlay 可继续，板上验收等板
