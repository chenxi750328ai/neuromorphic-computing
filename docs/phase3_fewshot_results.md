...
## 从头训练 + 数据增强（`train_mnist_fewshot_aug.py`）

旋转±15° + 平移 + 缩放：

| K | 无增强 | 有增强 | Δ |
|---|--------|--------|---|
| 5 | 66.84% | **69.70%** | +2.86pp |
| 10 | 75.76% | 75.21% | −0.55pp |

结论：增强对 5-shot 有微弱帮助（+3pp），但不解根本问题——**从头训练少量样本仍远不如全量微调**。Phase3 v0 结论维持：小样本首选预训练+微调。

## 结论（Phase3 v0 收尾）

| 路线 | K | best acc | 方式 |
|------|---|----------|------|
| ANN 微调 | 5 | **96.32%** | Phase2 ckpt → 少量微调 |
| SNN 微调 | 5 | **89.23%** | Phase1 ckpt → 少量微调 |
| SNN 微调 | 10 | **92.98%** | 同上 |
| 从头 增强 | 5 | 69.70% | 旋转平移+50张 |
| 从头 无增强 | 10 | 76.12% | 100张原始 |

→ Phase3 v0 交付完毕；余下难点（小样本 SNN 从头、meta-learning）归 v2。

---

*下一步：Phase4 Atlas/FPGA 推理；PR #1 可合并。*
