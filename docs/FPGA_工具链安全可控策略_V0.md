...
- **进展视角**：[`FPGA_开发流程与工具一览_V0.md`](./FPGA_开发流程与工具一览_V0.md)（A–H 阶段矩阵）  
- **xc7z020 开源 bit**：blinky（软逻辑+PS7）综合→bit→上板 **已实测 PASS**（见 S2 / openXC7 证据）；**含 CARRY/`lif_step` 的开源 P&R 仍未通** → LIF 默认 bit 仍 Vivado  
- 本 V0 目标是：**逻辑可信（RTL+Verilator）优先**；开源实现链按阶段推进，禁止 pen 判死刑  
- **S2**：[`FPGA_S2_开源友好器件迁移评估_V0.md`](./FPGA_S2_开源友好器件迁移评估_V0.md)；ECP5 PoC 须总裁触发
