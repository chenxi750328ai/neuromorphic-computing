...
## 2. Phase4.2 目标（草案 KPI · 待钉死）

| ID | 门禁 | 草案阈值 | 状态 |
|----|------|----------|------|
| P4.2-LAT | 单张 e2e 墙钟（同 F7 网/板上） | **待总裁钉**：建议候选 ≤**100 ms/张**（研究演示）或相对 Atlas p50 同量级 | ☐ |
| P4.2-ACC | 功能不回退 | board_pred_match_rate=1.0（N≥20） | ☐ |
| P4.2-BENCH | 机读报告 | `docs/phase4_poc_evidence/fpga_rb_fullnet_*_perf.json` + 一页更新 | ☐ |
| P4.2-ROOT | 根因关闭 | MMIO 细粒度踢门不再占墙钟主导（或书面豁免） | ☐ |

**工程方向（非穷尽）**：DMA/AXI-stream · 减少 PS↔PL 往返 · PL 内调度 · 批处理。

**关单**：P4.2-* 全绿 + neuro-ci + IPD（另开 WO）后，才解除下表 STALL。
...
1. P4.2-LAT 阈值采用：☐ 100ms/张 ☐ 其它：____ ☐ 先相对 Atlas 再定绝对值  
2. 本 STALL 表：☐ 同意 ☐ 增删 ID：____  
