...
```text
  假说分工 → 链路 bench → 发现瓶颈？ ──是──► 改分工（非改单点脚本了事）
                │                    │
                └──否──► 填规格对照表 · 写场景化倾向结论 → 再谈是否关 Phase4
```

**推论**：Phase4.1 **不能**在「只做完双跳 daemon」时关口；须按全链路规格补通信/切分对照数据后，才能写**带场景边界的倾向性结论**（探索项目：路线不预先唯一规定）。
...
2. **不浪费** — 先拿得出可复现双跳数据；FPGA 入链按 [补数议程](./Phase4.1_探索规格与补数议程_V0.md) **对照实验**，不口头「optional 掉」  
...
[WSL 编排 + 4090 金标准]
    ...
    │  (2) 下发 payload ──────────────┐
    ▼                                  ▼
[Atlas: SNN OM 主推理]          [FPGA: 事件/LIF/累加切片]  ← 探索对照（含/不含都要出数）
    │  spike_counts / 中间态          │
    └──────────┬──────────────────────┘
               ▼
...
| 门禁 | 命令/产物 |
|------|-----------|
| G0 **规格对照后的倾向性结论** | 填齐 [补数议程](./Phase4.1_探索规格与补数议程_V0.md) 对照表 + 场景边界；**禁止**半程数据「定稿分工」 |
...
| FPGA | 链中切片（待定） | **入链未测**；列入补数议程 H1/H2，禁止「暂缓=结论」 | 无三跳数据 |
| Phase4 关口 | 单站 PR #5/#7 叠加 | **规格对照表齐 + 场景化倾向结论**（非半程定稿） | 见探索规格 V0 |
...
### 5.2 FPGA 入链对照（按补数议程 · 非口头可选）

1. Atlas 输出中间结果 → PYNQ 做约定切片（见议程 E4 切分 ID）。  
2. 与「仅 Atlas」同尺子对比延迟/精度。  
3. 产出：`distributed_bench_atlas_fpga_*.json` 并填对照表。
...
| **Phase4 整体关闭** | 补数议程对照表齐 + 全链路规格下的场景化结论 + 总裁确认；**禁止**双跳 daemon 单独关口 |
...
| B2 | 端到端 PoC 线 | 见 [探索规格](./Phase4.1_探索规格与补数议程_V0.md) **G-LAT**（暂定中位≤5 ms、95 分位≤10 ms） |
| B3 | bench 脚本与分支 | 同意继续；**尚未**因 daemon 而关 Phase4.1 |

---

*陈正共 · 2026-06-22 初稿 · 2026-07-14 与探索规格对齐自检*
