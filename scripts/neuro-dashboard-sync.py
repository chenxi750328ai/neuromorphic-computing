...
def parse_phase4_status() -> dict:
    """从 QA 记录与 atlas 留档推断 M4-2/M4-3 状态。"""
    out = {"s2b": "WARN/待跑", "m4_3": "⏳ 下一步", "m4_4": "待 M4-3"}
...
def build_president_brief(ms: dict, live: dict) -> dict:
    ...
    brief = {
        ...
        "summary": (
            ...
            f"Phase4 Atlas MNIST 推理{'已冒烟通过' if m4_3_done else '进行中'}，"
            f"整体完成约 {completed}%。"
        ),
        ...
        "phase4_milestones": [
            ...
            {"id": "M4-2", "title": "4090 复现 + 模型导出", "status": "✅ S1 PASS · S2b " + p4["s2b"]},
        ...
        "brief_for_president": (
            ...
            + " SNN 直导 ONNX 仍标 WARN（LIF 算子），PoC 按 TR2 预案走了 ANN 近似路径。"
            + " 下一步是 TR2 轻评审勾选「Atlas 优先」与 Phase4 分支合 main；日常工程陈正共自驱，不必您盯。"
        ),
        "president_actions": [
...
def render_board_md(ms: dict, live: dict) -> str:
    ...
    p4_line = "筹备中"
    if p4.get("status") == "active":
        suffix = " · M4-3 **PASS**" if p4_status["m4_3"].startswith("✅") else " · Atlas 优先"
